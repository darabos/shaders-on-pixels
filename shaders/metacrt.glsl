...
uniform COMPAT_PRECISION float posx;
uniform COMPAT_PRECISION float posy;
uniform COMPAT_PRECISION float targetx;
uniform COMPAT_PRECISION float targety;
uniform COMPAT_PRECISION float zoom;
uniform COMPAT_PRECISION float showSphere;
uniform COMPAT_PRECISION float sphereX;
...
vec2 Cam_GetViewCoordFromUV( const in vec2 vUV )
{
	...
	vWindow.x *= SourceSize.x / SourceSize.y;
...
vec2 Cam_GetUVFromWindowCoord( const in vec2 vWindow )
{
    ...
    vScaledWindow.x *= SourceSize.y / SourceSize.x;
...
{   
    ...
    vec2 vResolution = SourceSize.xy;
...
//    vTextureUV = floor(vTextureUV * vResolution * 2.0) / (vResolution * 2.0);
    
    Interference interference = GetInterference( vTextureUV );

    float noiseIntensity = 0.1;
    
    //vTextureUV.x += (interference.scanLineRandom * 2.0 - 1.0) * 0.025 * noiseIntensity;
    
    
    vec3 vPixelEmissive = textureLod( Source, (vTextureUV.xy * vec2(1.0, -1.0) + vec2(0.0, 1.)) * vec2(InputSize.xy / TextureSize.xy), 1.0 ).rgb;

    if (snow > 0.5) vPixelEmissive = clamp( vPixelEmissive + (interference.noise - 0.5) * 2.0 * noiseIntensity, 0.0, 1.0 );
...
SurfaceInfo Scene_GetSurfaceInfo( const in vec3 vRayOrigin,  const in vec3 vRayDir, SceneResult traceResult )
{
    ...
    surfaceInfo.vEmissive = vec3( 0.2 );
    //return surfaceInfo;
    ...
    {
        surfaceInfo.vAlbedo = vec3(0.1, 0.1, 0.1); 
...
SceneResult Scene_GetCRT( vec3 vScreenDomain, vec2 vScreenWH, float fScreenCurveRadius, float fBevel, float fDepth )
{
    ...
    resultScreen.vUVW.xy = (vScreenDomain.xy / vScreenWH) * 0.5 + 0.5;
...
SceneResult Scene_GetComputer( vec3 vPos )
{
    ...
    float x = max( abs( vPos.x ) - 0.2, 0.0 );
...
//    vPos.x = fract( vPos.x ) - 0.5;
	...
	SceneResult resultScreen = Scene_GetCRT( vPos - vScreenPos, vScreenWH, 0.75, 0.02, 0.1 );
...
if (showSphere > 0.5){
    resultSet.vUVW = vPos.xzy;
    resultSet.fDist = length(vPos - vec3(sphereX,sphereY,sphereZ)) - 0.075;
    resultSet.iObjectId = int(sphereMaterial);//MAT_CHROME;
    Scene_Union( result, resultSet );
}   
...
vec4 MainCommon( vec3 vRayOrigin, vec3 vRayDir )
{
    ...
    vec4 FragColor = vColorLinAndDepth;
    
    float fExposure = 2.0;
    
    FragColor.rgb *= fExposure;
    
    FragColor.a = vColorLinAndDepth.w;
...
//    }
    ...
    if ( index == 2 )
    {
        cam.vPos = vec3(posx,posy,zoom);
        cam.vTarget = vec3(targetx,targety,0.5);
        cam.fFov = 4.5;
    }
    if ( index == 3 )
    {
...
void main()
{
    vec2 vUV = (gl_FragCoord.xy / SourceSize.xy) * (TextureSize.xy / InputSize.xy); 
...
/*    
    if ( iMouse.z > 0.0 )
    {
        float fDist = 0.01 + 3.0 * (iMouse.y / SourceSize.y);

        float fAngle = (iMouse.x / SourceSize.x) * radians(360.0);
    	//float fElevation = (iMouse.y / SourceSize.y) * radians(90.0);
    	float fElevation = 0.15 * radians(90.0);    
...
*/    
    ...
    vec2 vJitterUV = vUV + cam.vJitter;// / SourceSize.xy;
	...
	Cam_StoreState( ivec2(0), cam, FragColor, ivec2(gl_FragCoord.xy) );  
} 
#endif
